...
TaskHandle_t Task2;
 ...
 //#define pass "lafamilia"// "arduino1" //galgalgal
 #define ssid "arbs"   
 ...
 const int PBU = 27; // Brightness UP touch sensor
 const int PBD = 15; // Brightness Down touch sensor
 int  colorindex = 0;
 int brightness = 200; // Deafult brightness setting
...
void loop_0 ( void * pvParameters ){
  ...
  // io.run(); is required by adafruit. it should always be present at the top of your loop
     ...
     else
     {
      ...
      switch (cmd) {
       ...
       case 'c': //clear the strip
        ...
        break;
       case 'B': // Restart BlueTooth
        Serial.println("BlueTooth Restarted"); //
        Serial.println(SerialBT.begin("RGB Lamp"));
        break;        
...
/*
 //Local Brightness Reading
    ...
    if (brightness < (analogRead(Local) -10) or brightness > (analogRead(Local) +10)) {
     Serial.printf("%s, %d \n", "analog reading Brigthness set to =", brightness);
     Serial.printf("%s, %d \n", "analog reading =", brightness);  
     OTATerminal.println(brightness)   
  ...
  */
   ...
   */
   ...
   int brightup = touchRead(PBU); // untriggered value is 60-70
   int brightdown = touchRead(PBD);// untriggered value is 60-70
  // Serial.printf("%s, %d \n", "Brightness Up Button Value =", touchRead(PBU));
   //Serial.printf("%s, %d \n", "Brightness Down Button Value =", touchRead(PBD));
   if (brightup < 10 and brightness != 250){
     brightness = brightness + 10;
     strip.setBrightness(brightness); //(up to 255)
     strip.show();
     Serial.printf("%s, %d \n", "Brightness up. Value =", brightness);
     delay(30);
     
   }
    
   if (brightdown < 20 and brightness != 10){
     brightness = brightness - 10;
     strip.setBrightness(brightness); //(up to 255)
     strip.show();
     Serial.printf("%s, %d \n", "Brightness Down. Value =", brightness);
     delay(30);
  ...
  // Serial.printf("%s, %d \n", "Touch Button Value =", touch);
   delay(500);
   if (touchRead(PB) < 10) { //measure value normally 62
     if (colorindex == 0) {
       Serial.printf("%s, %d \n", "touch value=", touchRead(PB));
       Serial.println("Color Cycle Restarted "); 
       OTATerminal.println("Color Cycle Restarted ");
     ...
     //OTATerminal.println(colorindex);
     colorindex++;
     ...
     if (brightness > 241) {brightness = 250;}  
  }
}
